...
export async function loGin(req :Request , res:Response , next:NextFunction): Promise<void> {
    ...
    try {
        ...
        const token = jwt.sign({userId:loged_user.getId()} , secret ,  {expiresIn:'40s'});
        ...
        try {
            ...
            if(result.rows.length>0){
                res.status(200).json({
                    ...
                    user : UserManager.returnUsers(req.user.getId()) ,
...
export async function createComments(req:Request , res:Response , next:NextFunction):Promise<void>{
    ...
    try {
        ...
        const comment = new Comments(content , userId , retrieve_post.rows[0].id);
        PostsManager.addPostManager(parseInt(post_id) , comment)
      
        //insert to db
        const result = await connection.getPool().query(insert_to_comments,[content , userId , retrieve_post.rows[0].id])
        
        if (result.rows.length > 0) {
            
            res.status(200).json({
                message: 'Comment created',
                comment: { id: result.rows[0].id, content, postId: post_id, userId },
